...
//Tests insert multiple item 
int unitTest6(int status) {
    int passed = 0;
    hashmap_t* test = hashmap_create(5);
    hashmap_insert(test,"cow","A common farm animal.");
    hashmap_insert(test,"pig","A farm animal.");
	
    if(hashmap_hasKey(test, "pig")==1 && hashmap_hasKey(test, "cow")==1) {
...
int unitTest7(int status) {
    ...
    if(hashmap_hasKey(test, "cow")==0 && hashmap_hasKey(test, "pig") == 0) {
      passed = 1;
    }

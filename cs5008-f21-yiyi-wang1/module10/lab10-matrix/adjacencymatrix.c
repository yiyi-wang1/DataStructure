...
void generateGraph(int* g){
    ...
    for(i=0; i < ROWS; i++){
		...
		//Each node is not connected to itself
            }else{
...
int isConnected(int* g, int node1, int node2){
    if(g[node2 * COLUMNS + node1] == 1 | g[node1 * COLUMNS + node2] == 1){
	return 1;
    }
    return 0;
}
...
int main(){
    ...
    printf("Node 4 connections:\n");
    int j;
    for(j =0; j < COLUMNS; ++j){
        printf("With node %d : %d\n",j, isConnected(&g_testgraph,4,j));
    }
